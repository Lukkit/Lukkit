...
public class PluginWizard extends Thread {
    ...
    public void run() {
        if (this.sender instanceof ConsoleCommandSender) {
            this.sender.sendMessage("Creating Lukkit plugins from the console isn't supported due to player-like " +
                    "chat being unavailable. Sorry, do it yourself.");
            return;
        }
        ...
        this.sender.sendMessage(ChatColor.YELLOW + "Are you happy with the plugin? " +
                "Last chance to scrap it before writing to disk.");
        ...
        while (!this.template.isFinalized()) {
            ...
            if (input.equalsIgnoreCase("y")) {
                ...
                try {
                    ...
                    this.sender.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + "There was an error writing " +
                            "the plugin.yml file. Check the console for more info.");
                    e.printStackTrace();
                    this.cleanup();
                    return;
                }
                ...
                this.template.setFinalized();
                this.sender.sendMessage(ChatColor.YELLOW + "You can do /lukkit dev new-plugin any time to " +
    ...
    private synchronized Step entry() {
        ...
        this.sender.sendMessage(ChatColor.GREEN + "You'll be asked a few questions and a " +
                "plugin will be generated by the end.");
        return Step.NAME;
    }
    ...
    private synchronized Step name() {
        ...
        this.sender.sendMessage(ChatColor.YELLOW + "Legal names are [a-z], [A-Z], [0-9], hypens (-) " +
                "and underscores (_).");
        this.sender.sendMessage(ChatColor.YELLOW + "It also must start with an uppercase or lowercase letter. " +
                "Makes the name tidier.");
    ...
    private synchronized Step description() {
        this.sender.sendMessage(ChatColor.YELLOW + "If you want to write this over multiple lines, " +
    ...
    @WizardStep(value = Step.VERSION, firstRunOutput = "Your version should look something like 1.0.0, " +
            "but you can do whatever really. Don't go too crazy.")
    ...
    private Step executeNextStep() {
        ...
        if (finalMethod != null) {
            try {
                ...
                this.sender.sendMessage(ChatColor.RED + "" + ChatColor.BOLD + "There was an error running " +
                        "one of the steps. More information can be found in the console.");
